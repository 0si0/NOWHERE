...
| 주요 기능 | 공간에 맞는 음악 추천, Challenge, 장소 알림, Music Map, Shall We Share|
| 주요 연동 | Spotify, Firebase, OpenAI API, Kakao Maps, Open-Meteo |

## 주요 기능  
** Spotify의 api기능을 모두 사용하려면 개인 사업자 인증이 필요해서, spotify api 사용을 최대한 줄이는 데모버전으로 개발 했습니다 **
...
Spotify Web API를 통한 직접 재생 제어는 Premium 계정, 활성 디바이스, OAuth 권한 등 실행 조건의 영향을 크게 받기 때문에,   
추천 원을 탭하면 Spotify URI를 생성하고 Spotify 앱 딥링크/외부 앱 실행 방식으로 재생을 위임하도록 구현했습니다.
...
일반 모드는 "NEXT:공모전"의 심사위원 전용으로 만들어진 모드로, Spotify api의 기능을 모두 사용할 수 있는 모드입니다.   
Spotify의 api가 개발자가 지정한 5명만 사용할 수 있도록 제한을 걸어두어서 완전한 기능은 일반모드로 체험할 수 있게 구현했습니다.  
데모모드는 api 없이도 뮤직지도 기능을 체험할 수 있도록 하는 모드입니다.  
사용자가 플레이리스트를 지정하면, 그 순서대로 곡이 재생되며 지도에 위치와 함께 표시됩니다. 그러나, 백그라운드 기록이 안되어서 화면이꺼지면 기록도 중단됩니다.  
...
개인 포트폴리오 및 NE:XT 공모전 제출 목적의 프로젝트입니다. 


## 느낀점

이번 프로젝트를 통해 외부 API를 활용한 서비스 개발에서는 단순히 “구현 가능한 기능인지”만 보는 것이 아니라, API 정책, 권한 범위, 배포 조건, 사용자 계정 조건까지 먼저 확인해야 한다는 점을 크게 배웠습니다.

처음에는 Spotify를 활용해 장소에 도착하면 음악이 자동으로 재생되는 기능을 기획했지만, 실제 개발 과정에서 Spotify API 정책과 개발 모드 제한, Premium 계정 조건 등으로 인해 초기 기획을 그대로 구현하기 어렵다는 것을 알게 되었습니다. 이 경험을 통해 앞으로는 기능을 기획할 때 사용하려는 API가 실제 서비스 환경에서도 안정적으로 동작할 수 있는지 먼저 검토해야겠다고 느꼈습니다.

또한 제한이 생겼을 때 기능을 단순히 포기하는 것이 아니라, 서비스의 핵심 경험을 유지할 수 있는 방향으로 다시 설계하는 과정의 중요성을 배웠습니다. 장소 도착 시 자동 재생 대신 알림을 띄우고, 사용자가 직접 터치해 Spotify 재생으로 이어지도록 변경한 것은 기술적 제약을 고려하면서도 “장소와 음악을 연결한다”는 NOWHERE의 핵심 가치를 유지하기 위한 선택이었습니다.

이번 프로젝트는 기능 구현뿐 아니라, 외부 플랫폼 정책과 사용자 경험 사이에서 현실적인 제품 설계를 고민해본 경험이었습니다. 앞으로 프로젝트를 진행할 때는 기획 단계에서 API 제약과 배포 가능성을 먼저 확인하고, 기술적으로 가능한 구현과 실제 사용자가 자연스럽게 받아들일 수 있는 UX 사이의 균형을 더 신중하게 고려하고자 합니다.
